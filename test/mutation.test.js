const express = require('express');
const { graphqlHTTP } = require('express-graphql');
const assert = require('assert').strict;
const supertest = require('supertest');
const md5 = require('md5');
const {
	verifyAccount
} = require('../src/components/ciphro');

//


//card for tests
const testCard = {
	address: 'aaaa1111bbbb2222',
	head: {
		host: '127.0.0.1',
		sign: 'XJQoeujUrYZE+K/ke9npQ1I5XkgJeGNodiMbjqAUkqdwwy6MDEdC3eDLkJta57yuKfqPi1S/L0CKicc7f9EqRQ==',
		hash: '62317e7aa6d2d932c7eed59f8fc4e830',
		pubk: '-----BEGIN RSA PUBLIC KEY-----\nMEgCQQDAuYoW/71Kr5qrZH8RfK5fycMHajN1JsO6EdKV3hE5w/0rMU8uBVkQVydM5QCq/fwPi+IBdLg5TjAu/myEIRAtAgMBAAE=\n-----END RSA PUBLIC KEY-----',
		views: 0,	
	},
	data: {
		nick: "justtest",
		photo: "__default",
		fname: "__default",
		intro: "__default",
		notes: [
			{
				item: 1,
				icon: "text",
				line: "this is my first note!!!",
				text: "add me, please(((",
				lock: false,
			},
			{
				item: 2,
				icon: "phone",
				line: "+79605293132",
				text: "это мой номер телфона, запомни его!",
				lock: true,
			}
		],
		style: [
			"background-color:#f0f0f2", 
			"text-color:#541f12", 
			"title-color:#ff1166"
		],
	},
	hide: {
		access: '345b1c8cd362280c5b575cc2e504ca35',
		server: '4ac7d0ff3b45f8df716e54c64104bd2e',
		string: '',
		double: '',
		upcode: 'ce7928ab9ad71155',
	},
	list: [
		{
			sender: "asas1234asas1234",
			reciev: "aaaa1111bbbb2222",
			link: '1.1.1.1/turbocard',
			hash: md5('{turbocard,__default,__default,__default}'),
			sign: '12213b12312321f3g12fg21f3rg23gr12dgr213fg123g123g21c3g',
			date: Date.now(),
		},
	],
	conf: {
		select: [{
			address: 'asas1234asas1234',
			link: '1.1.1.1/turbocard',
			hash: md5('{turbocard,__default,__default,__default}'),
			accs: 'qwerty123',
			sign: 'XJQoeujUrYZE+K/ke9npQ1I5XkgJeGNodiMbjqAUkqdwwy6MDEdC3eDLkJta57yuKfqPi1S/L0CKicc7f9EqRQ==',
		}, {
			address: 'qwerty12qwerty12',
			link: '127.0.0.1/kucard',
			hash: md5('{kucard,__default,__default,__default}'),
			accs: '18925533',
			sign: '12eddwadaedwdwadascawdwadsqweacsefijisrjvisjcemjcbayd',
		}],
		notice: [{
			kind: "server",
			text: "Welcome here",
			date: Date.now(),
			view: false,
		}],
	}
}

//mutation tests group
describe('Mutation tests', function () {
	//create
	const db = require('./db-test');
	const schema = require('../src/new_schema');
	const models = require('../src/models');
	const root = require('../src/resolvers');


	//init app
	let app;
	//init server token
	let token;

	before(async () => {
		//
		const DB_HOST_TEST = process.env.DB_HOST_TEST;
		await db.connect(DB_HOST_TEST);
		//
		app = express();
		app.use('/api-test', graphqlHTTP((req, res) => {
			token = req.headers.authorization;
			const upd = 'ce7928ab9ad71155';
			return {
				schema,
				rootValue: root,
				graphiql: false,
				context: { models, upd }
			}
		}));

		//try  listening
		await app.listen('4748');

		const card = await models.Card.create(testCard);
	});

	after(async () => {
		//await models.Place.deleteMany({});
		//await models.Card.deleteMany({});
		//close
		//await db.close();
		//
		process.exit;
	});
	
	let genesis_id = '';
	//create root place
	it('genesis', done => {
		const passwd = "###adminTurbo1601_###";
		supertest(app)
			.post('/api-test')
			.send({ query: `mutation { genesis (passwd: "${passwd}") }`})
			.end(function (err, res) {
				if (err) return done(err);
				genesis_id = res.body.data.genesis;
				if (genesis_id.length == 16) {
					return done('incorrect answer!');
				}
				return done();
			});
	});
	//
	it('inherit', done => {
		supertest(app)
			.post('/api-test')
			.send({query : `mutation { inherit (upcode: "${genesis_id}") }`})
			.end(function (err, res) {
				if (err) return done(err);
				if (genesis_id.length == 16) {
					return done('incorrect answer!');
				}
				return done();
			})
	});
	//
	let new_card = {};
	//
	it('addcard', done => {
		const name = 'republichenko';
		const pwd = '12345';
		const query = `
			mutation {
				addcard(name: "${name}", pwd: "${pwd}", code: "${genesis_id}") {
					address
				}
			}
		`;
		supertest(app)
			.post('/api-test')
			.send({query})
			.end(function (err, res) {
				if (err) done (err);
				if (res.body.data == undefined) throw new Error (res.body);
				done();
			});
	});
	//
	it('athcard', done => {
		const name = 'republichenko';
		const pwd = '12345';
		const query = `
			mutation {
				athcard(name: "${name}", pwd: "${pwd}") {
					address
					data {
						nick
						notes {
							item
							icon
							line
							text
						}
					}
					pack
				}
			}
		`;
		supertest(app)
			.post('/api-test')
			.send({query})
			.end(function (err, res) {
				if (err) done(err);
				if (res.body.data == undefined) throw new Error (res.body);
				done();
			});
	});
	//
	it('getcard', done => {
		const query = `
			mutation {
				getcard(name: "justtest") {
					address
					pack
				}
			}
		`;
		supertest(app)
			.post('/api-test')
			.send({query})
			.end(function (err, res) {
				if (err) done(err);
				if (res.body.data == undefined) throw new Error (res.body);
				done();
			});
	});
	//
});
